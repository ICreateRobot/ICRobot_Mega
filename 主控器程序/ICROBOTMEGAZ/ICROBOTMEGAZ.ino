...
void Motor_Control(void)
{
  if (data[1] == 0x10) //前进
  {
    motor1.run(-data[2]); /* value: between -255 and 255. */
    motor2.run(data[2]); /* value: between -255 and 255. */
    motor3.run(-data[2]);
    motor4.run(data[2]);
    Serial.println(33);
  }
...
//    motor4.run(-data[2]);
        motor1.run(-data[2]); /* value: between -255 and 255. */
        motor2.run(-data[2]-20); /* value: between -255 and 255. */
        motor3.run(data[2]);
        motor4.run(data[2]+20);
  }
  ...
  else if ((data[1] == 0x20) || (data[1] == 0)) //停止电机
  {
    //    motor1.stop();
    //    motor2.stop();
    //    motor3.stop();
...
void setup()
{
  ...
  //  servopulse(100);   //引用脉冲函数  20-160舵机角度
  //  servopulse1(90);   //引用脉冲函数  30-80舵机角度
  //灯带
